...
void handleSetting1()
{
  int servo17Val = readKeyValue(7);
  String servo17ValStr = String(servo17Val);

  int servo16Val = readKeyValue(6);
  String servo16ValStr = String(servo16Val);

  int servo15Val = readKeyValue(5);
  String servo15ValStr = String(servo15Val);

  int servo14Val = readKeyValue(4);
  String servo14ValStr = String(servo14Val);

  int servo13Val = readKeyValue(3);
  String servo13ValStr = String(servo13Val);

  int servo12Val = readKeyValue(2);
  String servo12ValStr = String(servo12Val);

  int servo11Val = readKeyValue(1);
  String servo11ValStr = String(servo11Val);

  int servo10Val = readKeyValue(0);
  String servo10ValStr = String(servo10Val);
  String content = "";

  content += servo17ValStr;
  content += "=";
  content += servo16ValStr;
  content += "=";
  content += servo15ValStr;
  content += "=";
  content += servo14ValStr;
  content += "=";
  content += servo13ValStr;
  content += "=";
  content += servo12ValStr;
  content += "=";
  content += servo11ValStr;
  content += "=";
  content += servo10ValStr;
...
void enableWebServer()
{
  ...
  server.on("/setting1", getMethod, handleSetting1);
...
void handleIndex()
{
	...
	content += "</tr>";
	content += "<tr height=19%>";
	content += "<td><button type=button style=background:#CFBFFF onclick=controlPm(13)>Dance1</button></td>"; //Dance1
	content += "<td><button type=button style=background:#CFBFFF onclick=controlPm(14)>Dance2</button></td>"; //Dance2
	content += "<td><button type=button style=background:#CFBFFF onclick=controlPm(15)>Dance3</button></td>"; //Dance3
	content += "</tr>";
	content += "</table>";
